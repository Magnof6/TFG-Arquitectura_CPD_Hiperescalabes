#DEFINICIÓN DE ESCENARIOS DE PRUEBA

from models import (
    RedElectrica,
    Subestacion,
    Transformador,
    UPS,
    STS,
    Busbar,
    SalaIT,
    ZonaIT,
    ConexionElectrica,
    FalloComponente,
    GrupoRedundancia,
    Generador,
    RecuperacionComponente,
)
from topology import TopologiaSistema
from engine import EstadoSimulacion

def escenario_base_ups ():
    # -----------------------------------------------------------------
    # 1. COMPONENTES
    # -----------------------------------------------------------------

    red = RedElectrica(
        id="red",
        nombre="Red",
        tipo="RedElectrica",
        estado="activo",
        criticidad=5,
        tiempo_recuperacion_s=300,
        potencia_disponible_mw=100,
        tension_kv=400,
        numero_lineas=2,
        nodo_origen="grid",
        es_reserva=False
    )

    sub = Subestacion(
        id="sub",
        nombre="Subestación",
        tipo="Subestacion",
        estado="activo",
        criticidad=5,
        tiempo_recuperacion_s=300,
        potencia_nominal_mva=100,
        tension_entrada_kv=400,
        tension_salida_kv=11,
        lineas_entrantes=2,
        lineas_salientes=2,
        num_transformadores=2,
        esquema_barras="simple",
        es_reserva=False
    )

    trafo = Transformador(
        id="trafo",
        nombre="Transformador",
        tipo="Transformador",
        estado="activo",
        criticidad=4,
        tiempo_recuperacion_s=200,
        subtipo="IT",
        potencia_nominal_kva=2000,
        tension_entrada_kv=11,
        tension_salida_kv=0.415,
        impedancia_pct=6,
        grupo_vectorial="Dyn11",
        refrigeracion="ONAN",
        es_reserva=False
    )

    ups = UPS(
        id="ups_1",
        nombre="UPS 1",
        tipo="UPS",
        estado="activo",
        criticidad=5,
        tiempo_recuperacion_s=100,
        subtipo="IT",
        tecnologia="VFI",
        potencia_nominal_kva=1000,
        potencia_nominal_kw=900,
        eficiencia_pct=95,
        autonomia_min_eol=5,
        vida_util_anios=10,
        tiempo_conmutacion_ms=10,
        bateria_tipo="Li-ion",
        bms_monitorizado=True,
        es_reserva=False
    )
    ups_2 = UPS(
        id="ups_2",
        nombre="UPS 2",
        tipo="UPS",
        estado="reserva",
        criticidad=5,
        tiempo_recuperacion_s=100,
        subtipo="IT",
        tecnologia="VFI",
        potencia_nominal_kva=1000,
        potencia_nominal_kw=900,
        eficiencia_pct=95,
        autonomia_min_eol=5,
        vida_util_anios=10,
        tiempo_conmutacion_ms=10,
        bateria_tipo="Li-ion",
        bms_monitorizado=True,
        es_reserva=True,
    )

    sts = STS(
        id="sts_1",
        nombre="STS",
        tipo="STS",
        estado="activo",
        criticidad=5,
        tiempo_recuperacion_s=50,
        corriente_nominal_a=2000,
        tiempo_transferencia_ms=10,
        fuente_preferida="red",
        fuente_respaldo="ups_1",
        es_reserva=False
    )

    bus = Busbar(
        id="bus_1",
        nombre="Busbar",
        tipo="Busbar",
        estado="activo",
        criticidad=4,
        tiempo_recuperacion_s=50,
        tension_v=415,
        corriente_nominal_a=2000,
        capacidad_kw=800,
        es_reserva=False
    )

    # -----------------------------------------------------------------
    # 2. SALA Y ZONA
    # -----------------------------------------------------------------

    sala = SalaIT(
        id="sala_1",
        nombre="Sala IT",
        estado="alimentado",
        potencia_objetivo_kw=500,
        potencia_actual_kw=500,
        numero_zonas=1,
        bloque_electrico_principal="bloque_1",
        bloque_electrico_respaldo="bloque_2",
        criticidad=5,
    )

    zona = ZonaIT(
        id="zona_1",
        nombre="Zona IT",
        tipo="critica",
        estado="alimentada",
        demanda_kw=500,
        prioridad=5,
        sala_it_id="sala_1",
        alimentacion_preferida="red",
        alimentacion_respaldo="ups_1",
    )

    # -----------------------------------------------------------------
    # 3. TOPOLOGÍA
    # -----------------------------------------------------------------

    nodos = {
        red.id: red,
        sub.id: sub,
        trafo.id: trafo,
        ups.id: ups,
        ups_2.id: ups_2,
        sts.id: sts,
        bus.id: bus,
        sala.id: sala,
        zona.id: zona,
    }

    conexiones = [
        ConexionElectrica("red", "sub"),
        ConexionElectrica("sub", "trafo"),
        ConexionElectrica("trafo", "ups_1"),
        ConexionElectrica("trafo", "ups_2",tipo="respaldo"),
        ConexionElectrica("ups_1", "sts_1"),
        ConexionElectrica("ups_2", "sts_1", tipo="respaldo"),
        ConexionElectrica("sts_1", "bus_1"),
        ConexionElectrica("bus_1", "sala_1"),
        ConexionElectrica("sala_1", "zona_1"),
    ]

    topologia = TopologiaSistema(nodos=nodos, conexiones=conexiones)

    # -----------------------------------------------------------------
    # 4. GRUPOS N+1 (mínimo ejemplo)
    # -----------------------------------------------------------------

    grupos = {
        "ups_group": GrupoRedundancia(
            id="ups_group",
            nombre="Grupo UPS",
            tipo_componente="ups",
            componentes_ids=["ups_1", "ups_2"],
            capacidad_necesaria_kw=500,
            n_requerido=1,
        )
    }

    # -----------------------------------------------------------------
    # 5. ESTADO INICIAL
    # -----------------------------------------------------------------

    estado = EstadoSimulacion(
        tiempo_actual_s=0,
        estado_global="operativo",
        componentes={
            red.id: red,
            sub.id: sub,
            trafo.id: trafo,
            ups.id: ups,
            ups_2.id: ups_2,
            sts.id: sts,
            bus.id: bus,
        },
        salas_it={sala.id: sala},
        zonas_it={zona.id: zona},
        cargas_it={},  # puedes añadir más adelante
        topologia=topologia,
        grupos_redundancia=grupos,
    )
    
    # -------------------------------------------------
    # EVENTOS
    # -------------------------------------------------
    
    eventos = [
        FalloComponente(
            id="fallo_red",
            tipo="FalloComponente",
            tiempo_s=10,
            duracion_s=0,
            objetivo_id="red",
            objetivo_tipo="RedElectrica",
            descripcion="Caída de red eléctrica",
            severidad=5,
            causa="Fallo externo",
            nuevo_estado="fallado",
        )
    ]
    return estado, eventos

def escenario_con_generador():
    # -------------------------------------------------
    # COMPONENTES
    # -------------------------------------------------

    red = RedElectrica(
        id="red",
        nombre="Red",
        tipo="RedElectrica",
        estado="activo",
        criticidad=5,
        tiempo_recuperacion_s=300,
        potencia_disponible_mw=100,
        tension_kv=400,
        numero_lineas=2,
        nodo_origen="grid",
        es_reserva=False
    )

    sub = Subestacion(
        id="sub",
        nombre="Subestación",
        tipo="Subestacion",
        estado="activo",
        criticidad=5,
        tiempo_recuperacion_s=300,
        potencia_nominal_mva=100,
        tension_entrada_kv=400,
        tension_salida_kv=11,
        lineas_entrantes=2,
        lineas_salientes=2,
        num_transformadores=2,
        esquema_barras="simple",
        es_reserva=False
    )

    trafo = Transformador(
        id="trafo",
        nombre="Transformador",
        tipo="Transformador",
        estado="activo",
        criticidad=4,
        tiempo_recuperacion_s=200,
        subtipo="IT",
        potencia_nominal_kva=2000,
        tension_entrada_kv=11,
        tension_salida_kv=0.415,
        impedancia_pct=6,
        grupo_vectorial="Dyn11",
        refrigeracion="ONAN",
        es_reserva=False
    )

    ups_1 = UPS(
        id="ups_1",
        nombre="UPS 1",
        tipo="UPS",
        estado="activo",
        criticidad=5,
        tiempo_recuperacion_s=100,
        subtipo="IT",
        tecnologia="VFI",
        potencia_nominal_kva=1000,
        potencia_nominal_kw=900,
        eficiencia_pct=95,
        autonomia_min_eol=5,
        vida_util_anios=10,
        tiempo_conmutacion_ms=10,
        bateria_tipo="Li-ion",
        bms_monitorizado=True,
        es_reserva=False
    )

    ups_2 = UPS(
        id="ups_2",
        nombre="UPS 2",
        tipo="UPS",
        estado="reserva",
        criticidad=5,
        tiempo_recuperacion_s=100,
        subtipo="IT",
        tecnologia="VFI",
        potencia_nominal_kva=1000,
        potencia_nominal_kw=900,
        eficiencia_pct=95,
        autonomia_min_eol=5,
        vida_util_anios=10,
        tiempo_conmutacion_ms=10,
        bateria_tipo="Li-ion",
        bms_monitorizado=True,
        es_reserva=True,
    )

    gen_1 = Generador(
        id="gen_1",
        nombre="Generador 1",
        tipo="Generador",
        estado="reserva",   # importante: no está activo al inicio
        criticidad=5,
        tiempo_recuperacion_s=300,
        subtipo="IT",
        potencia_nominal_kva=1250,
        potencia_nominal_kwe=1000,
        tension_salida_kv=0.415,
        frecuencia_hz=50,
        tiempo_arranque_s=20,   # entra antes de que se agote la UPS
        consumo_l_h=200,
        autonomia_h=12,
        combustible_tipo="diesel",
        es_reserva=True
    )
    gen_2 = Generador(
        id="gen_2",
        nombre="Generador 2",
        tipo="Generador",
        estado="reserva",   # importante: no está activo al inicio
        criticidad=5,
        tiempo_recuperacion_s=300,
        subtipo="IT",
        potencia_nominal_kva=1250,
        potencia_nominal_kwe=1000,
        tension_salida_kv=0.415,
        frecuencia_hz=50,
        tiempo_arranque_s=20,   # entra antes de que se agote la UPS
        consumo_l_h=200,
        autonomia_h=12,
        combustible_tipo="diesel",
        es_reserva=True
    )

    sts = STS(
        id="sts_1",
        nombre="STS",
        tipo="STS",
        estado="activo",
        criticidad=5,
        tiempo_recuperacion_s=50,
        corriente_nominal_a=2000,
        tiempo_transferencia_ms=10,
        fuente_preferida="red",
        fuente_respaldo="ups_1",
        es_reserva=False
    )

    bus = Busbar(
        id="bus_1",
        nombre="Busbar",
        tipo="Busbar",
        estado="activo",
        criticidad=4,
        tiempo_recuperacion_s=50,
        tension_v=415,
        corriente_nominal_a=2000,
        capacidad_kw=800,
        es_reserva=False
    )

    # -------------------------------------------------
    # SALA Y ZONA
    # -------------------------------------------------

    sala = SalaIT(
        id="sala_1",
        nombre="Sala IT",
        estado="alimentado",
        potencia_objetivo_kw=500,
        potencia_actual_kw=500,
        numero_zonas=1,
        bloque_electrico_principal="bloque_1",
        bloque_electrico_respaldo="bloque_2",
        criticidad=5,
    )

    zona = ZonaIT(
        id="zona_1",
        nombre="Zona IT",
        tipo="critica",
        estado="alimentada",
        demanda_kw=500,
        prioridad=5,
        sala_it_id="sala_1",
        alimentacion_preferida="red",
        alimentacion_respaldo="ups_1",
    )

    # -------------------------------------------------
    # TOPOLOGÍA
    # -------------------------------------------------

    nodos = {
        red.id: red,
        sub.id: sub,
        trafo.id: trafo,
        ups_1.id: ups_1,
        ups_2.id: ups_2,
        gen_1.id: gen_1,
        gen_2.id: gen_2,
        sts.id: sts,
        bus.id: bus,
        sala.id: sala,
        zona.id: zona,
    }

    conexiones = [
        ConexionElectrica("red", "sub"),
        ConexionElectrica("sub", "trafo"),
        ConexionElectrica("trafo", "ups_1"),
        ConexionElectrica("trafo", "ups_2", tipo="respaldo"),

        # ruta alternativa desde generador
        ConexionElectrica("gen_1", "sts_1", tipo="respaldo"),
        ConexionElectrica("gen_2", "sts_1", tipo="respaldo"),

        ConexionElectrica("ups_1", "sts_1"),
        ConexionElectrica("ups_2", "sts_1", tipo="respaldo"),
        ConexionElectrica("sts_1", "bus_1"),
        ConexionElectrica("bus_1", "sala_1"),
        ConexionElectrica("sala_1", "zona_1"),
    ]

    topologia = TopologiaSistema(nodos=nodos, conexiones=conexiones)

    # -------------------------------------------------
    # GRUPOS
    # -------------------------------------------------

    grupos = {
        "ups_group": GrupoRedundancia(
            id="ups_group",
            nombre="Grupo UPS",
            tipo_componente="ups",
            componentes_ids=["ups_1", "ups_2"],
            capacidad_necesaria_kw=500,
            n_requerido=1,
        ),
        #"generador_group": GrupoRedundancia(
        #    id="generador_group",
        #    nombre="Grupo Generadores",
        #    tipo_componente="generador",
        #    componentes_ids=["gen_1", "gen_2"],
        #   capacidad_necesaria_kw=500,
        #    n_requerido=1,
        #)
        #Está comentado porque al poner este grupo, los generadores no entran por ArranqueGenerador si no por EntradaReserva y genera un bug
    }

    # -------------------------------------------------
    # ESTADO
    # -------------------------------------------------

    estado = EstadoSimulacion(
        tiempo_actual_s=0,
        estado_global="operativo",
        componentes={
            red.id: red,
            sub.id: sub,
            trafo.id: trafo,
            ups_1.id: ups_1,
            ups_2.id: ups_2,
            gen_1.id: gen_1,
            gen_2.id: gen_2,
            sts.id: sts,
            bus.id: bus,
        },
        salas_it={sala.id: sala},
        zonas_it={zona.id: zona},
        cargas_it={},
        topologia=topologia,
        grupos_redundancia=grupos,
    )

    # -------------------------------------------------
    # EVENTOS
    # -------------------------------------------------

    eventos = [
        FalloComponente(
            id="fallo_red",
            tipo="FalloComponente",
            tiempo_s=10,
            duracion_s=0,
            objetivo_id="red",
            objetivo_tipo="RedElectrica",
            descripcion="Caída de red eléctrica",
            severidad=5,
            causa="Fallo externo",
            nuevo_estado="fallado",
        )
    ]

    return estado, eventos

def escenario_caida_y_retorno_red():
    estado, eventos = escenario_con_generador()
    eventos.append(
        RecuperacionComponente(
            id="recuperacion_red",
            tipo="RecuperacionComponente",
            tiempo_s=610,  # la red vuelve después de 10 minutos
            duracion_s=0,
            objetivo_id="red",
            objetivo_tipo="RedElectrica",
            descripcion="Retorno de red eléctrica tras 10 minutos de caída",
            severidad=2,
            causa="Recuperación del suministro externo",
            nuevo_estado="activo",
        )
    )
    return estado, eventos






